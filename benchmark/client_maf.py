...
def gen_pload(total_lora_num, lora_popular_file):
    f = open('/workspace/aaas-data/dataset/lmsys_data.json')
    data = json.load(f)

    if lora_popular_file != None:
        lora_popular = np.load(lora_popular_file)
        total_lora_num = len(lora_popular)
    else:
        lora_popular = [1.0 / total_lora_num] * total_lora_num
    print("LoRA Num: {}. Popular: {}".format(total_lora_num, lora_popular[:3]))

    # set random seed, then sameple from the dataset.
    random.seed(0)
    np.random.seed(0)
    ...
    while True:
        for entry_id, entry in enumerate(sample):
            ...
            res = {"inputs": x, 
                ...
                "lora_id": np.random.choice( total_lora_num, p=lora_popular )}
...
if __name__ == '__main__':
    ...
    parser.add_argument('--total_lora_num', type=int, default=100, help='number of total LoRA on an instance')
    parser.add_argument('--lora_popular_file', type=str, default=None, help='LoRA Popularity File')
    args = parser.parse_args()

    load = gen_pload(args.total_lora_num, args.lora_popular_file)
