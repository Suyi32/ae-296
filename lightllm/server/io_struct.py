...
class Req:
    def __init__(self, request_id, prompt_ids, sample_params: SamplingParams, lora_id: int = 0):
...
class Batch:
    def __init__(self, batch_id, reqs: List[Req]):
        ...
        # workaround padding
        max_req_prompt_length = max([ req.input_len for req in reqs ])
        for req in self.reqs:
            if max_req_prompt_length > req.input_len:
                req.prompt_ids = [2 for _ in range(max_req_prompt_length - req.input_len)] + req.prompt_ids
                req.input_len = len(req.prompt_ids)
